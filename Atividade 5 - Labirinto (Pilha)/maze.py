...
class Maze:
    ...
    def localizar_tesouro(self) -> bool:
        caminho = deque()
        mapeados = set()
        origem = self.get_init_pos_player()
        caminho.append(origem)

        while caminho:
            pos_atual = caminho.pop()

            if self.find_prize(pos_atual):
                print("Tesouro achado!")
                return True

            if pos_atual in mapeados:
                continue

            mapeados.add(pos_atual)
            self.mov_player(pos_atual)
            sleep(1)
            ...
            for dx, dy in direcoes:
                adj = (lin + dx, col + dy)
                if adj not in mapeados and self.is_free(adj):
                    caminho.append(adj)
